...
//    @AutoLogOutput
    ...
    public Command runShooterScore(Escalator.Position atPosition, double timout) {
        ...
        ).withName("runShooterScoreCommand").withTimeout(timout).finallyDo(this::stop);
    }
    ...
    public Command runShooterIntake() {
        ...
        ).withName("runShooterIntakeCommand").until(this::shooterHasCoral).finallyDo(this::stop);
//                .andThen(new RunCommand(() -> this.setDutyCycle(ShooterSpeeds.Intake)).withTimeout(0.1))
//                .andThen(this::stop);
    }

    public enum ShooterSpeeds {
        Intake(0.15), Stopped(0), ShootL2AndL3(0.4), ShootL4(0.75), ShooterL1(0.25);
